...
\[ \left[ q_{t+1} \right] = \left[ \alpha \right] \left[ q_t \right] \] spike emission probability\+: \[ s_{t} = Bernoulli( \eta(q_t) ) \] (note, while a Bernoulli emission process is shown here, this toolbox works with any probability density function)
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{comment}{// std::vector<std::vector<double>> trs = \{\{0.9,0.1\},\{.1,.9\}\}; //transition probabilities, alpha}}
\DoxyCodeLine{\textcolor{comment}{// std::vector<std::vector<double>> frs = \{\{0.9,0.1\},\{.2,.8\}\}; //emission probabilities, eta}}
